...
AppStatus TileMap::Initialise()
{
	...
	if (data.LoadTexture(Resource::IMG_TILES, "images/tail8x8.png") != AppStatus::OK)
	{
	...
	if (data.LoadTexture(Resource::IMG_ENEMIE_RED, "images/friendsEric.png") != AppStatus::OK)
	{
		return AppStatus::ERROR;
	}
	img_enemy_red = data.GetTexture(Resource::IMG_ENEMIE_RED);
...
// Otras implementaciones de funciones de la clase TileMap...

bool TileMap::TestCollisionWallDown(const AABB& box) const {
	int x0, x1, y;

	// Calcular las coordenadas de los tiles y el rango de tiles para comprobar la colisión
	x0 = box.pos.x / TILE_SIZE;
	x1 = (box.pos.x + box.width - 1) / TILE_SIZE;
	y = (box.pos.y + box.height - 1) / TILE_SIZE;

	// Iterar sobre los tiles dentro del rango horizontal
	for (int x = x0; x <= x1; ++x) {
		// Si encuentra un tile sólido, hay colisión
	...
	return CollisionY(box.pos + Point(0, box.height - 1), 1);


}

// Implementación de otras funciones de la clase TileMap...
...
bool TileMap::TestCollisionWallUp(const AABB& box) const {
	...
	// No hubo colisión con las paredes superiores
	return false;
}
...
{
	Point p(box.pos.x, box.pos.y); // Punto de control para la colisión hacia arriba
	int tile_y;

	if (CollisionY(p, box.width))
	{
		tile_y = p.y / TILE_SIZE;
		*py = (tile_y + 1) * TILE_SIZE; // Ajustar la posición del jugador en caso de colisión
		return true;
	}
	return false;
}